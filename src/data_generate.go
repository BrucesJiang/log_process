	...
	*/
	...
	for {
		for i := 0; i < 4; i ++ {
			...
			paths := []string{"/foo", "/bar", "/baz", "/qux", "/foo", "/bar", "/bar"}
			...
			line := fmt.Sprintf("172.0.0.12 - - [%s +0000] %s \"GET %s HTTP/1.0\" %d %d \"-\" \"KeepAliveClient\"" +
				" \"-\" - %.3f\n", dateTime, scheme, path, code, bytesSend, requestTime)


			_, err := file.Write([]byte(line))
			if err != nil {
				log.Panic(fmt.Sprintf("WriteToFile Error: %s", err.Error()))
			}
		}
	}
}
